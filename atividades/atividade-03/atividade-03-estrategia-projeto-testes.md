...
| T6 | Pedido vazio | Acionar "Finalizar Pedido" | Nenhuma mudança de estado: a ação não deve estar disponível e nenhum pedido deve ser registrado | Transição bloqueada — observada: sem itens, o botão "Finalizar Pedido" não é exibido |
| T7 | Pedido enviado | Acionar "Finalizar Pedido" novamente | Nenhuma mudança de estado: não deve ser gerado um segundo pedido com o mesmo conteúdo | Transição bloqueada — não verificada |

A transição T4 é a única da tabela em que o estado obtido não corresponde ao esperado: na exploração da Atividade 1, ao retornar ao mesmo restaurante o painel "Seu Pedido" não estava mais presente e o item adicionado não constava mais, sem nenhuma mensagem ao usuário. É essa divergência que origina o risco R02 e o caso CT03, que passa a servir como caso de confirmação e de regressão — ou seja, deve falhar no comportamento atual e passar depois da correção. As transições T1, T2, T3, T5 e T6 correspondem ao comportamento observado; a T6 é o mesmo comportamento verificado pelo CT01, derivado da regra 2 da tabela de decisão. A transição T7 fica documentada como bloqueio a verificar em uma próxima rodada: após a confirmação o painel "Seu Pedido" deixa de ser exibido, mas o acionamento repetido de "Finalizar Pedido" antes da confirmação, que poderia gerar um pedido duplicado, não foi testado.
...
**Leitura da matriz:** a funcionalidade "fazer pedido" foi analisada integralmente; foram identificados dois riscos, ambos de prioridade alta; foram aplicadas duas técnicas distintas, cada uma escolhida pela natureza do risco; os três casos de teste estão vinculados a um risco e a uma técnica, e nenhum dos dois riscos ficou sem caso de teste correspondente. Na tabela de decisão, a regra 3 é inalcançável pela interface e a regra 4 foi observada e atendida, conforme a seção 3.2. A transição T7 fica registrada como próximo caso, para que a lacuna fique explícita em vez de invisível.
